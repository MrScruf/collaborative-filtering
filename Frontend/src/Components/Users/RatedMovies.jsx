...
export default class RatedMovies extends Component {
    ...
    async componentDidMount() {
        ...
        axios.get(this.props.apiUrl + "/movies/users/rated/count", { cancelToken: this.source.token }).then(response => {
    ...
    async loadData() {
        const CancelToken = axios.CancelToken;
        this.source = CancelToken.source();
        axios.get(`${this.props.apiUrl}/movies/users/rated/?offset=${(this.state.page - 1) * this.state.itemsOnPage}&limit=${this.state.itemsOnPage}`, { cancelToken: this.source.token }).then(response => {
    ...
    handlePageClick = (data) => {
        ...
        this.props.history.push(`/profile?page=${selected + 1}`)
    ...
    />
        return (
            <section>
                <h2 className="text-2xl mb-2 font-semibold">My reviews: </h2>
                {paginate}
